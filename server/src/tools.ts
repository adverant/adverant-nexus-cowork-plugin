...
export const NEXUS_TOOLS: Tool[] = [
  ...
  // 2. Skill Synthesis
  {
    ...
    description:
      'Synthesize new skills from existing ones. Strategies: combine (merge capabilities), chain (sequential workflow), specialize (focused subset), generalize (abstract patterns), adapt (different domain).',
    inputSchema: {
      type: 'object',
      properties: {
        source_skill_ids: {
  ...
  // 3. Memory
  {
    name: 'nexus_memory',
    description:
      'Store and recall memories using Nexus GraphRAG. Actions: store (save memory), recall (semantic search), store_episode (temporal event), recall_episodes (search episodes).',
    inputSchema: {
      type: 'object',
      properties: {
        action: {
  ...
  // 4. Documents
  {
    ...
    description:
      'Manage documents with intelligent chunking and embeddings. Actions: store (save document), get (retrieve by ID), list (browse), ingest (from URL), validate_url (check URL), check_job (ingestion status).',
    inputSchema: {
      type: 'object',
      properties: {
        action: {
  ...
  // 5. Entities (Knowledge Graph)
  {
    name: 'nexus_entities',
    description:
      'Manage knowledge graph entities and relationships. Actions: store (create entity), query (search entities), get (by ID), history (entity evolution), hierarchy (entity tree), facts (about subject), cross_domain (search across domains).',
    inputSchema: {
      type: 'object',
      properties: {
        action: {
  ...
  // 6. Search & Retrieval
  {
    name: 'nexus_search',
    description:
      'Search across all memory types. Actions: search (unified search), retrieve (with strategy selection), enhanced (multi-source with reranking).',
    inputSchema: {
      type: 'object',
      properties: {
        action: {
  ...
  // 7. Agent Orchestration
  {
    name: 'nexus_agents',
    description:
      'Coordinate AI agents via MageAgent. Actions: orchestrate (auto-assign agents), collaborate (shared context), compete (ranked solutions), analyze (deep analysis), synthesize (combine sources), list (active agents), details (agent info), task_status (check task).',
    inputSchema: {
      type: 'object',
      properties: {
        action: {
  ...
  // 8. Plugin Dispatcher
  {
    name: 'nexus_plugin',
    description:
      'Execute any installed marketplace plugin tool. Actions: discover (list available plugin tools, optionally filter by plugin_slug), execute (run a plugin tool by name with arguments).',
    inputSchema: {
      type: 'object',
      properties: {
        action: {
  ...
  // 9. System
  {
    name: 'nexus_system',
    description:
      'System health and model management. Actions: health (service status), stats (memory statistics), model_stats (model usage), model_select (choose best model for task).',
    inputSchema: {
      type: 'object',
      properties: {
        action: {
  ...
  switch (toolName) {
    ...
    case 'nexus_skills': {
      switch (args.action) {
        ...
        case 'get':
          if (!args.skill_id) throw new Error('skill_id is required for get action');
          return client.getSkill(args.skill_id);
        case 'discover':
          if (!args.query) throw new Error('query is required for discover action');
          return client.discoverSkills(args.query, args.categories, args.min_relevance, args.limit);
        case 'match':
          if (!args.prompt) throw new Error('prompt is required for match action');
        ...
        case 'related':
          if (!args.skill_id) throw new Error('skill_id is required for related action');
    ...
    case 'nexus_memory': {
      switch (args.action) {
        ...
        case 'recall':
          if (!args.query) throw new Error('query is required for recall action');
          return client.recallMemory(args.query, args.limit, args.score_threshold);
        case 'store_episode':
          if (!args.content) throw new Error('content is required for store_episode action');
          return client.storeEpisode(args.content, args.type, args.metadata);
        case 'recall_episodes':
          if (!args.query) throw new Error('query is required for recall_episodes action');
    ...
    case 'nexus_documents': {
      switch (args.action) {
        ...
        case 'get':
          if (!args.document_id) throw new Error('document_id is required for get action');
          return client.getDocument(args.document_id, args.include_chunks);
        case 'list':
          return client.listDocuments(args.limit, args.offset);
        case 'ingest':
          if (!args.url) throw new Error('url is required for ingest action');
          return client.ingestUrl(args.url, { skip_confirmation: args.skip_confirmation });
        case 'validate_url':
          if (!args.url) throw new Error('url is required for validate_url action');
    ...
    case 'nexus_entities': {
      switch (args.action) {
        ...
        case 'get':
          if (!args.entity_id) throw new Error('entity_id is required for get action');
          return client.getEntity(args.entity_id);
        case 'history':
          if (!args.entity_id) throw new Error('entity_id is required for history action');
          return client.getEntityHistory(args.entity_id);
        case 'hierarchy':
          if (!args.entity_id) throw new Error('entity_id is required for hierarchy action');
          return client.getEntityHierarchy(args.entity_id);
        case 'facts':
          if (!args.subject) throw new Error('subject is required for facts action');
          return client.getFacts(args.subject);
        case 'cross_domain':
          if (!args.domains || !args.query) throw new Error('domains and query are required for cross_domain action');
    ...
    case 'nexus_agents': {
      switch (args.action) {
        case 'orchestrate':
          if (!args.task) throw new Error('task is required for orchestrate action');
          return client.orchestrate(args.task, args.max_agents, args.timeout);
        case 'collaborate':
          if (!args.objective) throw new Error('objective is required for collaborate action');
          return client.collaborate(args.objective, args.agents, args.iterations);
        case 'compete':
          if (!args.challenge) throw new Error('challenge is required for compete action');
          return client.competition(args.challenge, args.competitor_count, args.evaluation_criteria);
        case 'analyze':
          if (!args.topic) throw new Error('topic is required for analyze action');
          return client.analyze(args.topic, args.depth, args.include_memory);
        case 'synthesize':
          if (!args.sources) throw new Error('sources is required for synthesize action');
          return client.synthesizeInfo(args.sources, args.objective, args.format);
        case 'list':
          return client.listAgents();
        case 'details':
          if (!args.agent_id) throw new Error('agent_id is required for details action');
          return client.getAgentDetails(args.agent_id);
        case 'task_status':
          if (!args.task_id) throw new Error('task_id is required for task_status action');
    ...
    case 'nexus_plugin': {
      switch (args.action) {
        case 'discover': {
          ...
          let pluginTools = (inventory.pluginTools || []).map((pt: any) => ({
            name: pt.name,
          ...
          if (args.plugin_slug) {
            pluginTools = pluginTools.filter((t: any) =>
              t.plugin?.toLowerCase().includes(args.plugin_slug.toLowerCase())
            );
          }
          return {
            plugins: inventory.activePlugins?.map((p: any) => ({
              slug: p.pluginSlug,
        ...
        case 'execute': {
          if (!args.tool_name) throw new Error('tool_name is required for execute action');
